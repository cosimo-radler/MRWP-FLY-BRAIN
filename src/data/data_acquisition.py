...
# Update data directory to save real networks to the correct location
...
def save_network(G, network_type):
    ...
    """
    # Save to both locations for backward compatibility during transition
    ...
    nx.write_gexf(G, data_path)
    nx.write_gexf(G, gephi_path)
    
    print(f"Network saved to {data_path} and {gephi_path}")
...
def main():
    ...
    # Save networks
    print("\nSaving networks...")
    save_network(eb_network, "eb")
    save_network(fb_network, "fb")
    save_network(mb_kc_network, "mb_kc")
